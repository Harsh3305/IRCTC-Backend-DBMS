...
function createTrain(train_name, train_code, callback) {
    ...
    connection.connect((error) => {
        if (error) {
            ...
            const sql_query = `INSERT INTO TRAIN (TRAIN_NAME, TRAIN_CODE, DELAY) VALUES ? `;
...
function getTrainFromSourceDestinaiton(source_id, destination_id, callback) {
    ...
    connection.connect((error) => {
        if (error) {
            ...
            const sql_query = `SELECT TRAIN_ID FROM STATION_STATUS WHERE STATION_ID = '${SOURCE_STATION_ID}' INTERSECT SELEVT TRAIN_ID FROM STATION_STATUS WHERE STATION_ID = '${DESTINATION_STATION_ID}`;
            connection.query(sql_query, (error, result) => {
                if (error) {
                    callback({ error_code: 500, error_message: error.message });
                }
...
module.exports = { createTrain, getTrain, getTrainFromSourceDestinaiton, getAllTrain, delayTrain }